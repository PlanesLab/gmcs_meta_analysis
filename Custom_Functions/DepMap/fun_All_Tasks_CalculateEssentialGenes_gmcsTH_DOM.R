CalculateEssentialGenes_gmcsTH_DOM <-  function(gene.exp, 
                                                gMCS_Analysis, 
                                                sample.class, 
                                                gmcsTH_perc = 0.05, 
                                                nWorkers = 4,
                                                gMCS_order = 1,
                                                order1 = TRUE){
  ## FUNCTION TO COMPUTE gMCSTh - DepMap ###
  # Input #
  ...
  #' @param gene.ratio # [All Genes, Samples], Ratio of Expression/Expression Threshold
  ...
  if(gMCS_order <= 7){
    ...
    if(!order1){
      gMCSs.ENSEMBL.mat <- gMCSs.ENSEMBL.mat[which(rowSums(gMCSs.ENSEMBL.mat) > 1),]
    }
  }
  ...
  if(order1){
    ...
    seq_list             <- which(nchar(gMCS_Analysis$gMCSs.ENSEMBL.txt) == 32)[1]:
      which(nchar(gMCS_Analysis$gMCSs.ENSEMBL.txt) == (15*(gMCS_order+1)+(2*gMCS_order)))[1]-1
    gMCSs.ENSEMBL.txt    <- gMCS_Analysis$gMCSs.ENSEMBL.txt[seq_list]    # Genes per gMCS (text)
  }
  ...
  rownames(gene.exp) <- gene.exp[,1] # Cell Line Information goes in rownames
  gene.exp <- gene.exp[ ,-1] # Remove Cell Line Information
  gene_Names_Full <-  strsplit(colnames(gene.exp), "[.]") # Separate SYMBOL & ENSEMBL
  colnames(gene.exp) <- sapply(gene_Names_Full, function(x) x[length(x)]) # Take Only ENSEMBL
  
  gene.exp[is.na(gene.exp)] <- 0 # Eliminate nans
  gene.exp <- t(gene.exp) # Use standard notation for genes-samples
  ...
  obtain_1_2_3_genes_gMCS <- function(i = 1){
    ...
    if (length(gmcs)==1){
      ...
      matched_gene_exp <- gene.exp[match(gmcs, rownames(gene.exp)),]
      gMCSs.ENSEMBL.exp <- rbind(as.matrix(matched_gene_exp), aux_zeros_mat)
    }
  ...
  list.gMCS.essential.mat <- function(g){Matrix::Matrix(gene.first.ENSEMBL==g & essential.single.gMCS, sparse = T)}
  list.gMCS.essential.mat <- foreach(g = genes.gMCSs.ENSEMBL[rowSums(mat.essential.gene)>0],
                                     .options.snow = opts) %dopar% list.gMCS.essential.mat(g) 
  ...
  ResultsEssentiality <- list(gene.ratio = gene.ratio,           # [All Genes, Samples] Ratio of Expression/Expression Threshold
                              ...
                              list.gMCS.essential.mat = list.gMCS.essential.mat) # List made of essential genes, in which a sparse matrix is in with [gMCS x CellLine]
  
  
  
  return(ResultsEssentiality)
