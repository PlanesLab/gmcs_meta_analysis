LocalT2CalculateEssential <- function(gene.exp, gMCS.info,
  ...
  ## FUNCTION TO COMPUTE LocalT2 - DepMap ###
  # Input #
  ...
  for (cm in levels(sample.cohort)){
    ...
    if (localT2_mode=="all_genes_gMCSs"){
      gene.cohort.TPM <- gene.exp[intersect(genes.gMCSs.ENSEMBL,rownames(gene.exp)), sample.cohort==cm]
    } else {
      gene.cohort.TPM <- gene.exp[intersect(table.genes.HumanGEM$ENSEMBL,rownames(gene.exp)), sample.cohort==cm]
    }
  ...
  if (localT2_mode=="all_genes_gMCSs"){
    gene.ON.OFF <- gene.exp[intersect(genes.gMCSs.ENSEMBL,rownames(gene.exp)),]*0
  } else {
    gene.ON.OFF <- gene.exp[intersect(table.genes.HumanGEM$ENSEMBL,rownames(gene.exp)),]*0
  }
  ...
  for (cm in levels(sample.cohort))
  {
    ...
    if (localT2_mode=="all_genes_gMCSs"){
      gene.aux.TPM <- gene.exp[intersect(genes.gMCSs.ENSEMBL,rownames(gene.exp)), sample.cohort==cm]
    } else {
      gene.aux.TPM <- gene.exp[intersect(table.genes.HumanGEM$ENSEMBL,rownames(gene.exp)), sample.cohort==cm]
    }
  ...
  results <- list() 
  results <- foreach(i=1:nn) %dopar% calculateNumberONgenes(i)
  # for(i in 1:nn) { 
  ...
  obtain_essential_single_gMCS_multiple_class <- function(i)
  {
    ...
    if (sum(aux1)==0){
      ...
      colnames(aux5) <- c(colnames(aux5)[1:3], colnames(aux3))
  ...
  NumberOfColumns <- ncol(list.gMCS.essential.mat[[1]])
  for(EssentialGene in genes.ENSEMBL.essential){
    list.gMCS.essential.mat[[EssentialGene]][which(unlist(gMCS.info$gMCSs.ENSEMBL.list) == EssentialGene)[1],] <- rep(TRUE,NumberOfColumns)
  }
